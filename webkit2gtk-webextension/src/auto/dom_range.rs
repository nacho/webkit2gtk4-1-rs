...
pub trait DOMRangeExt: IsA<DOMRange> + sealed::Sealed + 'static {
    ...
    fn collapsed(&self) -> Result<(), glib::Error> {
        unsafe {
            let mut error = ptr::null_mut();
            let is_ok =
                ffi::webkit_dom_range_get_collapsed(self.as_ref().to_glib_none().0, &mut error);
    ...
    fn intersects_node(&self, refNode: &impl IsA<DOMNode>) -> Result<(), glib::Error> {
        unsafe {
            let mut error = ptr::null_mut();
            let is_ok = ffi::webkit_dom_range_intersects_node(
                self.as_ref().to_glib_none().0,
                refNode.as_ref().to_glib_none().0,
                &mut error,
            );
    ...
    fn is_point_in_range(
        ...
        unsafe {
            let mut error = ptr::null_mut();
            let is_ok = ffi::webkit_dom_range_is_point_in_range(
                self.as_ref().to_glib_none().0,
                refNode.as_ref().to_glib_none().0,
                offset,
                &mut error,
    ...
    fn connect_common_ancestor_container_notify<F: Fn(&Self) + 'static>(
        ...
        unsafe extern "C" fn notify_common_ancestor_container_trampoline<
            P: IsA<DOMRange>,
            F: Fn(&P) + 'static,
        >(
            this: *mut ffi::WebKitDOMRange,
